    ...
    this.on('input', function (msg, send, done) {
      ...
      const decode = function (deviceid, reply) {
        if (reply) {
          ...
          if (response && (response.deviceId === deviceId) && response.state) {
            return response
          }
        }

        throw new Error(`no reply to get-status request for device ID ${deviceId}`)
      }
      ...
      try {
        const request = codec.encode(0x20, deviceId)
